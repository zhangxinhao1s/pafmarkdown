...
__webpack_require__.d(__webpack_exports__, {
  "default": function() { return /* binding */ indentLeftSyntaxPlugin; }
});
 ...
 */
function indentLeftSyntaxPlugin(context, options) {
  ...
  var toolbarItem = {
    name: 'indentLeft',
    tooltip: '减少缩进',
    className: `indent-left toastui-editor-toolbar-icons indent-left`,
    command: 'indentLeft',
  };

  return {
    wysiwygCommands: {
      indentLeft: function (_a, _b, dispatch) {
        ...
        tr.doc.nodesBetween(from, to, function(node, pos) {
          if (node.isBlock) {
            ...
            if (nodeAttrs.style) {
              var styles = nodeAttrs.style.split(';').filter(Boolean);
            ...
            indentLevel = currentIndent > 0 ? currentIndent - 20 : 0;
            if (indentLevel <= 0) {
              delete attrs['padding-left'];
            } else {
              attrs['padding-left'] = `${indentLevel}px`;
            }
            ...
            var newAttrs = { ...nodeAttrs, style: styleStr };
