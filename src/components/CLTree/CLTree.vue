      ...
      >
        ...
        <span v-else>
          <img
            ...
            alt=""
            v-if="data.datamode == 'juan'"
          />
          <img src="@/assets/icons/456.png" alt="" v-if="data.children == null" />
          <img
            src="@/assets/icons/123.png"
            alt=""
            v-if="data.datamode == 'mulu'"
          />
          <span style="margin-left:12px" v-if="data.jdlx != 4">{{node.label}}</span>
          <span style="margin-left:24px" v-if="data.jdlx == 4">{{node.label}}</span>
...
export default {
  ...
  watch: {
    data: {
      handler(val, preVal) {
        if (val && val.length > 0 && this.defaultSelFirst) {
          this.$nextTick(() => {
            this.$refs.tree1.$el.firstElementChild.click(); //拿到tree的第一个dom元素模拟一个点击事件
    ...
    currentNodeKey: {
      handler(val, oldVal) {
        setTimeout(() => {
          //延时一下，避免在某些场景未展开时会无效
          this.$refs["tree1"] && this.$refs["tree1"].setCurrentKey(val);
        }, 400);
      },
    },
    filterText(val) {
  ...
  methods: {
    expandHandle(val) {
      console.log('expandHandle--val----',val)
    ...
    getDeptDatas(node, resolve) {
      console.log("getDeptDatas---node---",this.data,node);
      // this.$emit("twoTreeData", node.data.id);
      if (node.data.length !=0 && node.isLeaf == false) {
        (node.loading = false), (node.expanded = true);
        this.$emit("twoTreeData", node.data.id,node);
      } else {
        node.expanded = true;
        node.loading = false;
      }
      // if (node.data.leaf == false && node.isLeaf == false) {
      //   (node.loading = false), (node.expanded = true);
